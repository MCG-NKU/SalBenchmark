...
int main(int argc,char *argv[])
{
	...
	for (int i = 0; i < imgNum; i++){
		if (CmFile::FilesExist(salDir + namesNE[i] + "_MSS.png"))
			continue;
		...
		sal.ComputeMaximumSymmetricSurroundSaliency(img, width, height, salmap, true);//true normalizes the saliency map
		...
		picHand.SavePicture(outimg, width, height, namesNE[i], salDir, 0, "_mss");// 0 is for BMP and 1 for JPEG)
		
		Mat sal1u = imread(salDir + namesNE[i] + "_mss.bmp", CV_LOAD_IMAGE_GRAYSCALE);
		imwrite(salDir + namesNE[i] + "_MSS.png", sal1u);
		CmFile::RmFile(salDir + namesNE[i] + "_MSS.bmp");
	}
}