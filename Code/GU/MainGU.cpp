// GU.cpp : Defines the entry point for the console application.
//
...
int main(int argc,char *argv[])
{
	...
	for (int i = 0; i < imgNum; i++){
		if (CmFile::FilesExist(salDir + namesNE[i] + "_GU.png"))
			continue;

		Mat img = imread(imgDir + namesNE[i] + imgExt);
		CV_Assert(img.data != NULL);
		img.convertTo(img, CV_32FC3, 1.0/255);
		CmSaliencyGC sGC(img, "", false);
		sGC.HistgramGMMs();
		Mat sal1f = sGC.GetSaliencyCues();
		imwrite(salDir + namesNE[i] + "_GU.png", sal1f*255);
	}
}